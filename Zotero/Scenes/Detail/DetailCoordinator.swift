...
final class DetailCoordinator: Coordinator {
        ...
        case .file(let filename, let contentType, _, _, _):
            ...
            case "text/html":
                if FeatureGates.enabled.contains(.htmlEpubReader) {
                    ...
                    showHTML(at: url, key: attachment.key, parentKey: parentKey, libraryId: libraryId)
                } else {
                    showWebView(for: url)
                }
            ...
            case "application/epub+zip":
                if FeatureGates.enabled.contains(.htmlEpubReader) {
                    ...
                    showEpub(at: url, key: attachment.key, parentKey: parentKey, libraryId: libraryId)
                } else {
    ...
    func createHtmlEpubController(key: String, parentKey: String?, libraryId: LibraryIdentifier, url: URL) -> DetailNavigationViewController {
        let navigationController = DetailNavigationViewController()
        navigationController.modalPresentationStyle = .fullScreen
    ...
    private func showHTML(at url: URL, key: String, parentKey: String?, libraryId: LibraryIdentifier) {
        showReaderItem(kind: .html(libraryId: libraryId, key: key)) {
            self.createHtmlEpubController(key: key, parentKey: parentKey, libraryId: libraryId, url: url)
        }
    }

    private func showEpub(at url: URL, key: String, parentKey: String?, libraryId: LibraryIdentifier) {
...
extension DetailCoordinator: OpenItemsPresenter {
    func showItem(with presentation: ItemPresentation?) {
        ...
        case .html(let library, let key, let parentKey, let url):
            showHTML(at: url, key: key, parentKey: parentKey, libraryId: libraryId)

        case .epub(let library, let key, let parentKey, let url):
            showEpub(at: url, key: key, parentKey: parentKey, libraryId: libraryId)
