...
final class AnnotationsViewController: UIViewController {
    ...
    private func showFilterPopup(from barButton: UIBarButtonItem) {
        ...
        self.coordinatorDelegate?.showFilterPopup(from: barButton, filter: self.viewModel.state.filter, availableColors: sortedColors, availableTags: sortedTags, userInterfaceStyle: self.viewModel.state.interfaceStyle, completed: { [weak self] filter in
