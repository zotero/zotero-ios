...
final class SyncControllerSpec: QuickSpec {
    override class func spec() {
        describe("a sync controller") {
            let userId = 100
            let userLibraryId: LibraryIdentifier = .custom(.myLibrary)
            ...
            beforeEach {
                ...
                realmConfig = Realm.Configuration(inMemoryIdentifier: UUID().uuidString)
                ...
                syncController.set(coordinator: TestConflictCoordinator(createZoteroDirectory: true))
            }
            ...
            describe("Syncing") {
                ...
                context("Download") {
                    it("should download items into a new library") {
                        ...
                        objects.forEach { object in
                            createStub(
                                for: ObjectsRequest(libraryId: libraryId, userId: userId, objectType: object, keys: (objectKeys[object] ?? "")),
                                baseUrl: baseUrl,
                                headers: header,
...
//                                expect(condition?.value).to(equal("thesis"))
                    ...
                    it("should download items into a new read-only group") {
                        ...
                        let myLibrary = userLibraryId
                        
                        createStub(for: GroupVersionsRequest(userId: userId), baseUrl: baseUrl, headers: header,
                                   jsonResponse: groupVersionsResponse)
                        objects.forEach { object in
                            createStub(for: VersionsRequest(libraryId: myLibrary, userId: userId, objectType: object, version: 0),
                        ...
                        for object in objects {
                            createStub(for: VersionsRequest(libraryId: libraryId, userId: userId, objectType: object, version: 0),
                                       baseUrl: baseUrl, headers: header, jsonResponse: (versionResponses[object] ?? [:] as [String: Any]))
                        }
                        for object in objects {
                            createStub(for: ObjectsRequest(libraryId: libraryId, userId: userId, objectType: object, keys: (objectKeys[object] ?? "")),
                                       baseUrl: baseUrl, headers: header, jsonResponse: (objectResponses[object] ?? [:] as [String: Any]))
                        }
                        createStub(for: KeyRequest(), baseUrl: baseUrl, url: Bundle(for: Self.self).url(forResource: "test_keys", withExtension: "json")!)
                        createStub(
                            for: SettingsRequest(libraryId: myLibrary, userId: userId, version: 0),
                            baseUrl: baseUrl,
                            headers: header,
...
//                    let realm = realm
//                    try! realm.write {
//                        let myLibrary = realm.objects(RCustomLibrary.self).first
//                        let item = RItem()
//                        item.key = itemToDelete
...
//                    let request = UpdatesRequest(libraryId: libraryId, userId: userId, objectType: .item, params: [], version: 0)
...
//                        createStub(for: VersionsRequest(libraryId: libraryId, userId: userId,
//                                                                     objectType: object, version: version),
//                                        baseUrl: baseUrl, headers: header,
//                                        response: [:])
//                    }
//                    createStub(for: KeyRequest(), baseUrl: baseUrl, response: ["access": ["":""]])
//                    createStub(for: SettingsRequest(libraryId: libraryId, userId: userId, version: 0),
//                                    baseUrl: baseUrl, headers: header,
//                                    response: ["tagColors" : ["value": [], "version": 2]])
//                    createStub(for: DeletionsRequest(libraryId: libraryId, userId: userId, version: 0),
//                                    baseUrl: baseUrl, headers: header,
//                                    response: ["collections": [], "searches": [], "items": [itemToDelete], "tags": []])
//
//                    self.controller = SyncController(userId: userId,
...
//                }
                    ...
                    it("should include unsynced objects in sync queue") {
                        ...
                        createStub(for: KeyRequest(), baseUrl: baseUrl, url: Bundle(for: Self.self).url(forResource: "test_keys", withExtension: "json")!)
                        createStub(
                            for: ObjectsRequest(libraryId: libraryId, userId: userId, objectType: .item, keys: "\(unsyncedItemKey),\(responseItemKey)"),
                            baseUrl: baseUrl,
                            headers: header,
                            jsonResponse: itemResponse
                        )
                        createStub(
                            for: ObjectsRequest(libraryId: libraryId, userId: userId, objectType: .item, keys: "\(responseItemKey),\(unsyncedItemKey)"),
                            baseUrl: baseUrl,
                            headers: header,
                            jsonResponse: itemResponse
                        )
                    ...
                    it("should mark object as needsSync if not parsed correctly and syncRetries should be increased") {
                        ...
                        createStub(for: KeyRequest(), baseUrl: baseUrl, url: Bundle(for: Self.self).url(forResource: "test_keys", withExtension: "json")!)
                        createStub(
                            for: ObjectsRequest(libraryId: libraryId, userId: userId, objectType: .item, keys: "\(correctKey),\(incorrectKey)"),
                            baseUrl: baseUrl,
                            headers: header,
                            jsonResponse: itemResponse
                        )
                        createStub(
                            for: ObjectsRequest(libraryId: libraryId, userId: userId, objectType: .item, keys: "\(incorrectKey),\(correctKey)"),
                            baseUrl: baseUrl,
                            headers: header,
                            jsonResponse: itemResponse
                        )
...
//                                expect(search2?.conditions.count).to(equal(0))
                    ...
                    it("should add items that exist remotely in a locally deleted, remotely modified collection back to collection") {
                        ...
                        createStub(
                            for: SubmitDeletionsRequest(libraryId: libraryId, userId: userId, objectType: .collection, keys: [collectionKey], version: 0),
                            baseUrl: baseUrl,
                            headers: header,
                            statusCode: 412,
                            jsonResponse: [:] as [String: Any]
                        ...
                        waitUntil(timeout: .seconds(60)) { doneAction in
                            syncController.reportFinish = { result in
                                ...
                                case .success(let data):
                                    ...
                                    syncController.start(type: .prioritizeDownloads, libraries: .specific([libraryId]), retryAttempt: 4)
                                }
                            }
                    ...
                    it("should add locally deleted items that exist remotely in a locally deleted, remotely modified collection to sync queue and remove from delete log") {
                        ...
                        createStub(for: KeyRequest(), baseUrl: baseUrl, url: Bundle(for: Self.self).url(forResource: "test_keys", withExtension: "json")!)
                        createStub(
                            for: SubmitDeletionsRequest(libraryId: libraryId, userId: userId, objectType: .collection, keys: [collectionKey], version: 1),
                            baseUrl: baseUrl,
                            headers: header,
                            statusCode: 412,
                            jsonResponse: [:] as [String: Any]
                        ...
                        createStub(
                            for: SubmitDeletionsRequest(libraryId: libraryId, userId: userId, objectType: .item, keys: [deletedItemKey], version: 1),
                            baseUrl: baseUrl,
                            headers: header,
                            statusCode: 412,
                            jsonResponse: [:] as [String: Any]
                        ...
                        objects.forEach { object in
                            createStub(
                                for: VersionsRequest(libraryId: libraryId, userId: userId, objectType: object, version: 1),
                                baseUrl: baseUrl,
                                headers: header,
                                jsonResponse: versionResponses[object] ?? [:] as [String: Any]
                            )
                        ...
                        createStub(
                            for: SettingsRequest(libraryId: libraryId, userId: userId, version: 1),
                            baseUrl: baseUrl,
                            headers: header,
                            jsonResponse: ["tagColors": ["value": [["name": "A", "color": "#CC66CC"]], "version": 1] as [String: Any]]
                        )
                ...
                context("Upload") {
                    it("should update collection and item") {
                        ...
                        let collectionUpdate = UpdatesRequest(libraryId: libraryId, userId: userId, objectType: .collection, params: [], version: oldVersion)
                        ...
                        let itemUpdate = UpdatesRequest(libraryId: libraryId, userId: userId, objectType: .item, params: [], version: oldVersion)
                        // We don't care about specific params, we just want to catch update for all objecfts of this type
                        ...
                        waitUntil(timeout: .seconds(60)) { doneAction in
                            ...
                            syncController.start(type: .normal, libraries: .specific([.custom(.myLibrary)]), retryAttempt: 0)
                        }
                    }
                    ...
                    it("should upload child item after parent item") {
                        ...
                        let update = UpdatesRequest(libraryId: libraryId, userId: userId, objectType: .item, params: [], version: oldVersion)
                    ...
                    it("should upload child collection after parent collection") {
                        ...
                        let update = UpdatesRequest(libraryId: libraryId, userId: userId, objectType: .collection, params: [], version: oldVersion)
                        let conditions = update.stubCondition(with: baseUrl)
                        stub(condition: conditions, response: { request -> HTTPStubsResponse in
                            guard let params = request.httpBodyStream.flatMap({ self.jsonParameters(from: $0) }) else {
                                fail("parameters not found")
                    ...
                    it("should process downloads after upload failure") {
                        ...
                        stub(condition: SettingsRequest(libraryId: libraryId, userId: userId, version: 0).stubCondition(with: baseUrl),
                    ...
                    it("should download remote file after file upload failure") {
                        ...
                        createStub(
                            for: AuthorizeUploadRequest(libraryId: libraryId, userId: userId, key: key, filename: filename, filesize: UInt64(data.count), md5: fileMd5, mtime: 123, oldMd5: nil),
                            ignoreBody: true,
                            baseUrl: baseUrl,
                            statusCode: 412,
                            jsonResponse: [:] as [String: Any]
                        ...
                        createStub(
                            for: UpdatesRequest(libraryId: libraryId, userId: userId, objectType: .item, params: [], version: 0),
                            baseUrl: baseUrl,
                            headers: header,
                            jsonResponse: [:] as [String: Any]
                        )
                        ...
                        createStub(for: SettingsRequest(libraryId: libraryId, userId: userId, version: 0), baseUrl: baseUrl, headers: header, jsonResponse: [:] as [String: Any])
                        createStub(for: KeyRequest(), baseUrl: baseUrl, url: Bundle(for: Self.self).url(forResource: "test_keys", withExtension: "json")!)
                        createStub(
                            for: DeletionsRequest(libraryId: libraryId, userId: userId, version: 0),
                            baseUrl: baseUrl,
                    ...
                    it("should upload local deletions") {
                        ...
                        createStub(
                            for: SubmitDeletionsRequest(libraryId: libraryId, userId: userId, objectType: .search, keys: [searchKey], version: 0),
                            baseUrl: baseUrl,
                            headers: header,
                            jsonResponse: [:] as [String: Any]
                        )
                        createStub(
                            for: SubmitDeletionsRequest(libraryId: libraryId, userId: userId, objectType: .item, keys: [itemKey], version: 0),
                            baseUrl: baseUrl,
                            headers: header,
                            jsonResponse: [:] as [String: Any]
                        )
...
//                }
                ...
                context("full sync") {
                    ...
                    it("should download missing/updated local objects and flag remotely missing local objects for upload") {
                        ...
                        createStub(for: GroupVersionsRequest(userId: userId), baseUrl: baseUrl, headers: nil, statusCode: 200, jsonResponse: [:] as [String: Any])
                        createStub(
                            for: ObjectsRequest(libraryId: libraryId, userId: userId, objectType: .item, keys: "\(outdatedKey),\(locallyMissingKey)"),
                            baseUrl: baseUrl,
                            headers: header,
                        ...
                        waitUntil(timeout: .seconds(60)) { doneAction in
                            syncController.reportFinish = { result in
                                ...
                                case .success:
                                    ...
                                    expect(remotelyMissingItem?.changedFields).toNot(equal([]))
                            ...
                            syncController.start(type: .full, libraries: .all, retryAttempt: 0)
                        }
                    }
                    ...
                    it("shouldn't check for remote changes if some write action succeeded to reach zotero backend") {
                        ...
                        createStub(
                            for: RegisterUploadRequest(libraryId: libraryId, userId: userId, key: key, uploadKey: "key", oldMd5: nil),
                            baseUrl: baseUrl,
                            headers: nil,
                            statusCode: 204,
                            jsonResponse: [:] as [String: Any]
                ...
                context("WebDAV") {
                    it("Creates zotero directory if missing and continues upload") {
                        ...
                        webDavController.sessionStorage.isEnabled = true
                        webDavController.sessionStorage.scheme = .http
                        webDavController.sessionStorage.url = "test.com"
                        webDavController.sessionStorage.isVerified = false
                        webDavController.sessionStorage.username = "user"
                        webDavController.sessionStorage.password = "password"
    ...
    private class func jsonParameters(from stream: InputStream) -> [[String: Any]] {
        let json = try? JSONSerialization.jsonObject(with: stream.data, options: .allowFragments)
        return (json as? [[String: Any]]) ?? []
    }

    private class func itemJson(key: String, version: Int, type: String) -> [String: Any] {
