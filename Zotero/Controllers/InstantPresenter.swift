...
protocol InstantPresenter: AnyObject {
    ...
    func show(viewControllerProvider: () -> UIViewController, by presenter: UIViewController, in window: UIWindow, animated: Bool, completion: (() -> Void)?)
}

extension InstantPresenter {
    func show(viewControllerProvider: () -> UIViewController, by presenter: UIViewController, in window: UIWindow, animated: Bool, completion: (() -> Void)? = nil) {
        DDLogInfo("InstantPresenter: show controller; animated=\(animated)")
        
        if animated {
            let controller = viewControllerProvider()
        ...
        show(presentedViewController: screenshotController, by: presenter, in: window) { presenter, completion in
            let controller = viewControllerProvider()
            if presenter.presentedViewController == nil {
                DDLogInfo("InstantPresenter: no presented controller, present controller")
