...
class CitationController: NSObject {
    ...
    func startSession(for itemIds: Set<String>, libraryId: LibraryIdentifier, styleId: String, localeId: String, webView: WKWebView? = nil) -> Single<Session> {
        ...
        return loadStyleData(controller: self, for: styleId)
            ...
            .flatMap { styleXML, styleLocaleId, localeXML, supportsBibliography -> Single<(String, String, String, Bool, String)> in
                return loadSchema().flatMap { .just((styleXML, styleLocaleId, localeXML, supportsBibliography, $0)) }
            }
            .flatMap { [weak self] styleXML, styleLocaleId, localeXML, supportsBibliography, schema -> Single<(String, String, String, Bool, String, String)> in
                guard let self else { return .error(Error.deinitialized) }
                return loadItemJsons(controller: self, for: itemIds, libraryId: libraryId)
                    .flatMap { .just((styleXML, styleLocaleId, localeXML, supportsBibliography, schema, $0)) }
            }
            .flatMap { styleXML, styleLocaleId, localeXML, supportsBibliography, schema, itemJsons -> Single<(String, String, String, Bool, String)> in
